...
class RandomPlayer:
    ...
    def __init__(self):
        ...
        self.RingPos = {}
        self.movesMap = self.game.boardToHexMap(self.n)
        self.rings_left = 3
        self.play()
    ...
    def defense(self, move, player, movesMap) :
        ...
        return (max_len*1.0 - num_markers*0.001, max_len_b)
    ...
    def selectAndMoveRing(self):
        ...
        for move in validMoves:
            move_defense, opp_len = self.defense(move, self.player, self.movesMap)
            if (opp_len > 3) :
                f = False
                if (minDefense > move_defense) : # and the move has a higher defense than any previous valid moves
                    src_ring = self.movesMap[(move[0], move[1])]
                    dst_ring = self.movesMap[(move[2], move[3])]
                    minDefense = move_defense # update move and defense score

        for move in validMoves:
            move_aggression = self.aggression(move, self.player, self.movesMap)
            if ((maxAggress < move_aggression and f) or (self.rings_left == 1 and move_aggression > 5)) : # and the move has a higher aggression than any previous valid moves
                src_ring = self.movesMap[(move[0], move[1])]
                dst_ring = self.movesMap[(move[2], move[3])]
